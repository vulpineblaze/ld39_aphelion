...
function enemySpawn(x,y, enemies, gravity, maxEnergy){
		var enemy = enemies.create(x, y, 'enemy'+enemies.index);
...
function generateWave(stars,enemies){
    ...
    {
		var enemy = enemySpawn(randomIntFromInterval(1,waveWidthCount)*waveWidthSpacing, -typicalWidth*i-verticalOffset, enemies, baseGravity/2,enemies.maxEnergy);
    }
}
function generateSecondGenWave(secondGenEnemies){
	...
	for (var i = 1; i < waveCount; i++)
    {
		var enemy = enemySpawn(randomIntFromInterval(1,waveWidthCount)*waveWidthSpacing, -typicalWidth*i-verticalOffset, secondGenEnemies, baseGravity/2,secondGenEnemies.maxEnergy);
    }
}

function levelChecker(game, stars, enemyArray, timer, flavorState, flavorText){
		if (game.time.totalElapsedSeconds() > 0 && flavorState == "start"){
			...
			generateWave(stars,enemyArray[0]);
			...
			flavorState = "wave4";
			generateWave(stars,enemyArray[0]);
			generateSecondGenWave(enemyArray[1]);
